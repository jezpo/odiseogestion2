                                    ...
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <form id="crearNuevoDocumentoForm" class="form-horizontal" method="POST"
                                                enctype="multipart/form-data" action="{{ route('documentosReci.store') }}">
                                                @csrf
    ...
    function deleteDocument(docId) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: "{{ route('documentosReci.destroy', ['id' => '']) }}" + doc_id,
                    beforeSend: function() {
...
<script>
    function editDocument(id) {
        $.get("{{ route('documentosReci.edit', ['id' => '']) }}" + id, function(data) {
            $('#txtId2').val(data.id);
            $('#cite2').val(data.cite);
    ...
    $('#editDocumentoForm').submit(function(e) {
        ...
        $.ajax({
            url: "{{ route('documentosReci.update', ['id' => '']) }}" + id2, // Asegúrate de que esta URL es correcta
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
...
<script>
    ...
    function loadPDF(id) {
        $.ajax({
            url: "{{ route('documentosReci.download', ['id' => '']) }}" + id,
            method: 'GET',
            success: function(response) {
